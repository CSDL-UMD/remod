...
def get_nodevectors_vector(nodevectors_model: Type[KeyedVectors], node: Type[nx.Graph.node]) -> np.array:
    node_str = None
...
def get_n2v_vector(n2v_model: Type[gensim.models.Word2Vec], node: Type[nx.Graph.node]) -> np.array:
    node_str = node_to_str(node)
